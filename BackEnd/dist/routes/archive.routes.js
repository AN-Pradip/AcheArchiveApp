...
archiveRouter.get('/your-archive/:id', token_authentification_1.default, (req, res) => {
    database_connection_1.default.getConnection((err, conn) => {
        //If connection error
        if (err) {
            console.log(err);
        ...
        database_connection_1.default.query(`SELECT * FROM Pain WHERE User_idUser = '${req.body.Userid}' AND idPain=?`, [req.params.id], (err, table) => {
            if (err) {
                conn.release();
                return res.send({
                    success: false,
